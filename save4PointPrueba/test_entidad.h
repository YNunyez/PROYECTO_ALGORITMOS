...
class entidad{
	...
	public:
		...
		void atacar();	
		// Funci�n para serializar una entidad en un archivo binario
	    void serializar(std::ofstream& archivo) {
	        // Serializar los datos relevantes de la entidad en el archivo
	    }
	
	    // Funci�n para deserializar una entidad desde un archivo binario
	    void deserializar(std::ifstream& archivo) {
	        // Deserializar los datos relevantes de la entidad desde el archivo
	    }	//SE ENCARGARÁ DE LOS ENFRENTAMIENTOS(INCOMPLETO DE MOMENTO)
};
