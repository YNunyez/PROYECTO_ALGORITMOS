...
int main(){
	string ruta_archivo = "C:\\Doc\\Programacion\\PROYECTO_ALGORITMOS-main\\PROYECTO_ALGORITMOS-main";
	...
	for(int piso=1;piso<11;piso++){
		...
		while(o[0]+o[1]<20){
		    ...
		    std::ofstream archivo_salida("ruta_archivo", std::ios::binary);
		    if (archivo_salida.is_open()) {
		        // Serializar los datos relevantes del juego
		        // Por ejemplo:
		        // nodoDoble.serializar(archivo_salida);
		        // listaDoble.serializar(archivo_salida);
		        // entidad.serializar(archivo_salida);
		        archivo_salida.close();
		        cout << "La partida se ha guardado correctamente en " << ruta_archivo << endl;
		    }
			else {
		    	std::cerr << "No se puedo abrir el archivo." << endl;
			}
		
		    // Cargar el estado del juego
		    std::ifstream archivo_entrada("ruta_archivo", std::ios::binary);
		    if (archivo_entrada.is_open()) {
		        // Deserializar los datos relevantes del juego
		        // Por ejemplo:
		        // nodoDoble.deserializar(archivo_entrada);
		        // listaDoble.deserializar(archivo_entrada);
		        // entidad.deserializar(archivo_entrada);
		        archivo_entrada.close();
		        cout << "EL juego se ha cargado correctamente desde:" << ruta_archivo << endl;
		    }
		    else {
